...
local events = Window:AddTab({ Title = "events", Icon = "moon" })
...
local craftAll = false
local eventselecteditems = {}
...
local event = events:AddSection("Moon coins farm")
event:AddToggle("AutofarmToggle", {
    Title = "Autofarm Blood Tokens ( normal)",
    Default = false,
...
event:AddToggle("farmblood", { 
    Title = "farm blood coins and dust (op) (use before patch)",
    Default = false,
    Callback = function(state)
    farmblood = state
...
event:AddToggle("autoactive",{
    Title = " auto activate machine",
    Default = false,
    Callback = function(state)
    autoactive = state
    print("auto activate machine toggle:", state )
    end 
})
event:AddToggle("autodust",{
    Title = "auto dust",
    Default = false,
    Callback = function(state)
    autodust = state
...
local craftevent = events:AddSection("crafting")

local craftingRemotes = game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CraftingEvents")
local craftList = {
    "blood_throne",
    "blood_tail",
    "blood_lamp",
...
local craftDropdown = craftevent:AddDropdown("CraftDropdown", {
    Title = "Select Items",
    Values = craftList,
    Multi = true,
    Default = {}
...
-- ✅ FIX: Properly handle dictionary format
craftDropdown:OnChanged(function(values)
    local cleaned = {}
    ...
    eventselecteditems = cleaned

    print("Selected items to craft:", #eventselecteditems > 0 and table.concat(eventselecteditems, ", ") or "None")
end)

-- Toggle for auto-crafting selected
craftevent:AddToggle("AutoCraftToggle", {
    Title = "Auto Craft Selected",
    Default = false,
    Callback = function(state)
        autocraft = state
    end
...
-- Toggle for crafting all
craftevent:AddToggle("CraftAllToggle", {
    Title = "Craft All",
    Default = false,
    Callback = function(state)
        craftAll = state
...
-- Loop to run crafting
spawn(function()
    while task.wait(5) do
        if craftAll then
            for _, item in ipairs(craftList) do
                craftingRemotes:WaitForChild("Craft"):InvokeServer(item)
            end
        elseif autocraft then
            for _, item in ipairs(eventselecteditems) do
                craftingRemotes:WaitForChild("Craft"):InvokeServer(item)
            end
        end
    end
