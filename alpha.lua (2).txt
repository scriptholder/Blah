...
function Tab:AddDrop(DropTitle, DropIcon)
    ...
    -- resolve optional icon via library, same pattern as Tab:AddSection
    local Icon = DropIcon
    if not fischbypass then
        if Library:GetIcon(Icon) then
            Icon = Library:GetIcon(Icon)
    ...
    -- Root container that sits in the tab’s list (same parent as sections)
    Drop.Root = New("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 0, 34), -- header height when closed
        Parent = Tab.Container,
        LayoutOrder = 7,
    })

    -- Header (button) – themed colors
    Drop.Header = New("TextButton", {
        ...
        New("ImageLabel", {
            ...
            LayoutOrder = 3,
            Size = UDim2.fromOffset(16, 16),
            Image = Library:GetIcon and (Library:GetIcon("chevron_down") or "") or "",
            Rotation = 0,
            ThemeTag = { ImageColor3 = "Text" },
        }),
    })

    local Chevron = Drop.Header.Chevron

    -- Content holder (clips, animates height)
    Drop.ContentHolder = New("Frame", {
        BackgroundTransparency = 1,
    ...
    -- Inner stack where sections will be inserted
    Drop.Container = New("Frame", {
        BackgroundTransparency = 1,
        AutomaticSize = Enum.AutomaticSize.Y,
        Size = UDim2.new(1, -12, 0, 0),
        Position = UDim2.fromOffset(6, 6),
        Parent = Drop.ContentHolder,
    }, {
        New("UIListLayout", {
    ...
    local layout = Drop.Container:FindFirstChildOfClass("UIListLayout")
    local headerHeight = 34
    local paddingY = 12 -- inside padding around sections

    local function setSizes(openInstant)
        local targetContent = Drop.Open and (layout.AbsoluteContentSize.Y + paddingY) or 0
        if openInstant then
            Drop.ContentHolder.Size = UDim2.new(1, -10, 0, targetContent)
            Drop.Root.Size = UDim2.new(1, 0, 0, headerHeight + targetContent + (Drop.Open and 2 or 0))
            Chevron.Rotation = Drop.Open and 180 or 0
        else
            TweenService:Create(Drop.ContentHolder, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { Size = UDim2.new(1, -10, 0, targetContent) }):Play()
            TweenService:Create(Chevron, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { Rotation = Drop.Open and 180 or 0 }):Play()
            TweenService:Create(Drop.Root, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { Size = UDim2.new(1, 0, 0, headerHeight + targetContent + (Drop.Open and 2 or 0)) }):Play()
        end
    end

    -- keep height in sync when sections inside change
    ...
    -- open/close
    Creator.AddSignal(Drop.Header.MouseButton1Click, function()
        Drop.Open = not Drop.Open
        setSizes(false)
    end)

    -- public: add a Section inside this drop (same metatable as normal section)
    function Drop:AddSection(SectionTitle, SectionIcon)
        local Section = { Type = "Section" }

        local Icn = SectionIcon
        if not fischbypass then
            if Library:GetIcon(Icn) then
        ...
        -- if the drop is open, grow to fit immediately
        if Drop.Open then setSizes(false) end
        return Section
    end
...
-- NEW: fallback label for empty search
...
NoResultsLabel.Parent = DropdownHolderFrame
...
Creator.AddSignal(DropdownDisplay.InputBegan, function(input)
	if input.UserInputType == Enum.UserInputType.Keyboard then
		local kc = input.KeyCode
		if kc == Enum.KeyCode.Down then
			local vis = clampActive()
			if #vis > 0 then
				Dropdown.ActiveIndex += 1
				clampActive()
				ensureInView(getVisibleButtons()[Dropdown.ActiveIndex])
			end
		...
		elseif kc == Enum.KeyCode.Return or kc == Enum.KeyCode.KeypadEnter then
			local vis = clampActive()
			local btn = vis[Dropdown.ActiveIndex] or vis[1]
			if btn and btn:FindFirstChild("ButtonLabel") then
				Dropdown:_selectValue(btn.ButtonLabel.Text)
			end
...
ElementsTable.Slider = (function()
	...
	function Element:New(Idx, Config)
		...
		-- NEW: return visible buttons in display order
local function getVisibleButtons()
	local out = {}
	for _, inst in ipairs(DropdownScrollFrame:GetChildren()) do
		if inst:IsA("TextButton") and inst.Visible ~= false then
			table.insert(out, inst)
		end
	end
	table.sort(out, function(a,b) return a.LayoutOrder < b.LayoutOrder end)
	return out
end

-- NEW: keep active index for keyboard nav
Dropdown.ActiveIndex = 1

local function clampActive()
	local vis = getVisibleButtons()
	if #vis == 0 then
		Dropdown.ActiveIndex = 0
	else
		if Dropdown.ActiveIndex < 1 then Dropdown.ActiveIndex = 1 end
		if Dropdown.ActiveIndex > #vis then Dropdown.ActiveIndex = #vis end
	end
	return vis
end

-- NEW: ensure the active item is scrolled into view
local function ensureInView(btn)
	local top = DropdownScrollFrame.CanvasPosition.Y
	local bottom = top + DropdownScrollFrame.AbsoluteWindowSize.Y
	local y = btn.AbsolutePosition.Y - DropdownScrollFrame.AbsolutePosition.Y
	local yEnd = y + btn.AbsoluteSize.Y
	if y < top then
		DropdownScrollFrame.CanvasPosition = Vector2.new(0, y)
	elseif yEnd > bottom then
		DropdownScrollFrame.CanvasPosition = Vector2.new(0, yEnd - DropdownScrollFrame.AbsoluteWindowSize.Y)
	end
end

-- NEW: select/toggle a value programmatically (used by Enter key)
function Dropdown:_selectValue(value)
	if Config.Multi then
		...
		if Dropdown.Value == value and Config.AllowNull then
			Dropdown.Value = nil
		else
			Dropdown.Value = value
		end
	...
	-- UX: close for single-select on Enter
	if not Config.Multi then
		Dropdown:Close()
	end
end

-- NEW: filter list (debounce not required; TextChanged is cheap)
function Dropdown:Filter(query)
	query = (query or ""):lower()
	local visibleCount = 0
	for _, inst in ipairs(DropdownScrollFrame:GetChildren()) do
		if inst:IsA("TextButton") then
			local label = inst:FindFirstChild("ButtonLabel")
			local val = label and label.Text:lower() or ""
			local show = (query == "") or (val:find(query, 1, true) ~= nil)
			inst.Visible = show
